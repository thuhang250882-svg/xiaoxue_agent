...
def fetch_trending(period: str = "weekly", limit: int = 25,
                   language: str = "", topic: str = "", token: str = None) -> list:
    ...
    topics = [topic] if topic and topic != "ai" else []
    if topic == "ai":
        topics = ["ai", "llm", "agent", "rag", "machine-learning"]

    queries = [f"pushed:>={since} stars:>=10{lang_filter}"] if not topics else [
        f"{item} in:name,description pushed:>={since} stars:>=10{lang_filter}" for item in topics
    ]
...
def main():
    ...
    parser.add_argument(
        "--topic",
        default="",
        help="主题过滤；使用 ai 查询 AI/ML/LLM/Agent/RAG 项目",
    )
    parser.add_argument(
        "--token", "-t",
    ...
    repos = fetch_trending(args.period, args.limit, args.language, args.topic, args.token)
