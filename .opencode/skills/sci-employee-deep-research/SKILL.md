...
受管内网模式禁止调用下文公网 IP 上的 Deep Research 服务及其中的外部模型。不得发送内部论文、报告正文、井名、项目代号、未公开研究主题或任何内部附件。默认改用当前已部署的本地模型组织用户提供的本地证据；需要公开文献时，只能发送经过用户确认的脱敏公开检索词。无法满足这些条件时仅输出 dry-run 请求计划，并明确标记外部研究服务已被策略阻止。
