...
# 投标文件生成（RC6 Business Skills）

## 定位

为投标人基于真实招标要求矩阵和真实企业资料生成投标章节草稿。本技能不替代审核 Skill 自证合格。

## 路由边界

- 审核招标/投标文件、提取废标红线、核查评分和偏差：`tender-document-review`。
- 为招标人编制招标技术要求、资格条件或评标办法：当前 RC6 不在范围内（参考已记录）。
- 为投标人基于真实要求和真实企业资料生成投标章节：本技能。

## Trigger 与 Not-Trigger

- **触发**：写标书、生成投标文件、编制投标响应、生成技术标/商务标草稿、生成响应章节。
- **不触发**：审核招标/投标文件（`tender-document-review`）、打分评估、招标方编制技术要求。
...
- **工具**：文档生成（`document_generation`）、`tender_review` 终审（自检）。
- **知识**：招标要求矩阵（来自 `tender-document-review` 输出）、企业真实资料。
- **关联 Skill**：`tender-document-review`（输入门槛 + 最终 QA）。
