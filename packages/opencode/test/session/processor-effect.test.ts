...
it.live("session.processor effect tests promote finished reasoning when no visible text is returned", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()

        yield* llm.push(reply().reason("reasoning-only answer").stop())
        ...
        expect(value).toBe("continue")
        expect(reasoning?.text).toBe("reasoning-only answer")
        expect(text?.text).toBe("reasoning-only answer")
      }),
    { config: (url) => providerCfg(url) },
  ),
)
