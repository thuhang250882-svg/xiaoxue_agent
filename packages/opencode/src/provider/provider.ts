...
// Merge registry-shaped providers onto config providers without mutating either
// input. Models from the registry win so an edited modelId replaces the stale one.
...
// Remove legacy config models that the registry now owns (imported entries and
// tombstones). Without this, deleted or edited legacy models would stay alive
// as a parallel call path straight from opencode.json.
function stripLegacyManaged(
  base: NonNullable<ConfigV1.Info["provider"]>,
  refs: Set<string>,
) {
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // Registry-managed models overlay onto config providers: the registry file is the
        // single source of truth for local models, so deleted entries truly disappear
        // while edited modelIds take effect on this rebuild without touching opencode.json.
        ...
        // builtin models the user disabled from the model manager; they stay in the
        // catalog but must not be loadable or listed
          ...
          Object.entries(s.providers).flatMap(([providerID, provider]) => {
            ...
            const models = Object.fromEntries(
              Object.entries(provider.models).filter(([modelID, model]) => {
                ...
                return XiaoxueEnterprisePolicy.allowsProviderNetwork(endpoint)
              }),
            )
