...
//
// Renders the full xiaoxue entry logo and a compact [X] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "录井小雪", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `xiaoxue --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
