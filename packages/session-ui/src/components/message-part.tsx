...
function CollapsibleMarkdown(props: { text: string; cacheKey: string; streaming: boolean }) {
  ...
  return (
    <>
      <Show when={props.streaming} fallback={<Markdown text={visible()} cacheKey={cacheKey()} streaming={false} />}>
      ...
      <Show when={overflow && !expanded()}>
        <div data-slot="text-part-expand" class="pt-1">
          <button type="button" class="text-12-regular text-text-weak hover:text-text-base" onClick={expand}>
      ...
      <Show when={overflow && expanded()}>
        <div data-slot="text-part-collapse" class="pt-1">
          <Show when={!renderingFull()} fallback={<span class="text-12-regular text-text-weak">正在展开…</span>}>
            <button
              type="button"
