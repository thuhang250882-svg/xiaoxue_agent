...
export type HomeSessionsViewProps = {
  ...
  onCreateSession: (prompt?: string, agent?: string, autoSubmit?: boolean) => void
        ...
        >
          ...
          >
            <div ref={props.onSetContent} class="flex flex-col pt-3 pr-3 pb-16">
              <HomeSessionsEmpty onNewSession={props.onCreateSession} language={props.language} />
              <div class={`${HOME_SECTION_LABEL} px-2 pb-3 pt-8`}>历史任务</div>
...
const XIAOXUE_WORKFLOWS = [
  ...
  },
  {
    ...
    agent: "knowledge",
    prompt: "请说明需要查询的制度、标准、模板或专业问题。我会优先检索本地资料并标明来源；没有可靠依据时会明确说明。",
  },
  {
    title: "文档生成",
...
function HomeSessionsEmpty(props: {
  ...
  return (
    <div class="flex min-h-full flex-col gap-5 px-2 pb-10 pt-[36px]">
      <div class="rounded-[8px] border border-v2-border-border-muted bg-v2-background-bg-layer-01 px-4 py-4">
        <img
          src="/assets/pet/xiaoxue-portrait-front.png"
          alt="录井小雪"
          class="mb-3 size-14 rounded-full object-cover object-top"
        />
        <div class="text-[15px] leading-5 text-v2-text-text-base [font-weight:560]">录井小雪工作台</div>
        <p class="mt-2 text-[13px] leading-5 text-v2-text-text-muted [font-weight:440]">
          桌宠默认操作为“显示小雪助手”。面向西部钻探录井工程分公司的企业级业务智能体，提供专业审核、企业办公、知识查询和文档生成。
        </p>
      </div>
      <div class="grid min-w-0 grid-cols-1 gap-2 xl:grid-cols-2">
        <For each={XIAOXUE_WORKFLOWS}>
          {(item) => (
            <button
              type="button"
              data-action={`xiaoxue-workflow-${item.agent}`}
              class="group flex min-h-[112px] min-w-0 flex-col justify-between rounded-[8px] border border-v2-border-border-muted bg-v2-background-bg-layer-01 px-4 py-3 text-left hover:border-v2-border-border-base hover:bg-v2-background-bg-layer-02 focus-visible:outline-none"
              onClick={() => props.onNewSession?.(item.prompt, item.agent, true)}
            >
              <div class="flex min-w-0 items-start gap-3">
                <span class="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-[6px] border border-v2-border-border-muted text-v2-icon-icon-muted">
                  <IconV2 name={item.icon} />
                </span>
                <span class="min-w-0 flex flex-col gap-1">
                  <span class="text-[13px] leading-4 text-v2-text-text-base [font-weight:560]">{item.title}</span>
                  <span class="text-[12px] leading-5 text-v2-text-text-muted [font-weight:440]">
                    {item.description}
                  </span>
                </span>
              </div>
              <span class="mt-3 text-[11px] leading-4 text-v2-text-text-muted [font-weight:530]">{item.detail}</span>
            </button>
          )}
        </For>
      </div>
