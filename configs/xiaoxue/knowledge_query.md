...
- **纯扫描件 PDF（无文字层）两步入库**：`knowledge_manage import` 报"纯扫描件"错误时，①用打包 Python 跑 `pdfkit.py extract_text --input <原PDF> --ocr_fallback`（内置 RapidOCR，离线），把全文合并保存为一个 .txt 文件；②再次调用 `knowledge_manage import`，同时传 `paths=[原PDF路径]` 与 `ocr_text_path=[txt路径]`。原始 PDF 与 OCR 文本会一并入库。禁止只做 OCR 归档（如 xiaoxue_obsidian_archive）而不入库。
